...
const AlumniVerification = () => {
  ...
  const statsData = [
    { title: 'Auto-approved', value: '247', change: '+12%', icon: '✅', color: 'bg-green-100 text-green-800' },
  ...
  const fetchPendingAlumni = async () => {
    try {
      setLoading(true);
      const response = await api.get('/institution/pending');
      setPendingAlumni(response.data.data);
    } catch (error) {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await api.get('/institution/notifications');
  ...
  const handleVerifyAlumni = async (alumniId, decision, method) => {
    try {
      await api.post(`/institution/verify/${alumniId}`, {
        method,
        decision
      });
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className={`mt-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                AI-powered verification of alumni credentials
              </p>
            </div>
            <div className="mt-4 md:mt-0 flex items-center space-x-3">
              ...
              <button className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:opacity-90 transition-all text-sm font-medium shadow-md">
                Export Data
              </button>
              <button 
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex space-x-2">
              ...
              <button 
                ...
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${filter === 'approved' ? (darkMode ? 'bg-green-700 text-white' : 'bg-green-100 text-green-800') : (darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700')}`}
              >
                Approved
              </button>
              <button 
                onClick={() => setFilter('rejected')}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${filter === 'rejected' ? (darkMode ? 'bg-red-700 text-white' : 'bg-red-100 text-red-800') : (darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700')}`}
              >
                Rejected
              </button>
            </div>
        ...
        >
          <div className={`px-6 py-4 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <h2 className="text-lg font-semibold">AI Verification Queue</h2>
            <p className={`text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              Pending alumni awaiting verification
            </p>
          </div>
          <div className="p-6">
            {loading ? (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(alumni.status)}`}>
                        {alumni.status === 'approved' || alumni.status === 'auto-approved' ? '✅' : 
                    ...
                    {alumni.aiScore && (
                      <div className="mt-4 space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className={darkMode ? 'text-gray-400' : 'text-gray-600'}>AI Score</span>
                    ...
                    <div className="mt-4 flex space-x-2">
                      ...
                      >
                        {alumni.status === 'approved' || alumni.status === 'auto-approved' ? 'Approved' : 
                ...
                <div className={`h-full flex flex-col ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-xl`}>
                    ...
                    <div className={`border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} px-4 sm:px-6`}>
                      <div className="mt-6">
                        ...
                        {selectedAlumni.aiScore && (
                          <div className="mt-6">
                            <div className="flex justify-between items-center">
                              <h4 className={`text-md font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>AI Verification Score</h4>
                        ...
                        <div className="mt-6">
                          <h4 className={`text-md font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Confidence Breakdown</h4>
                          ...
                          <div className={`mt-6 p-4 rounded-lg ${darkMode ? 'bg-indigo-900' : 'bg-indigo-50'}`}>
                            <div className="flex">
                              ...
                              <div className="ml-3">
                                <h3 className={`text-sm font-medium ${darkMode ? 'text-indigo-200' : 'text-indigo-800'}`}>AI Summary</h3>
                  ...
                  <div className={`border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} px-4 py-4 sm:px-6`}>
                    <div className="flex space-x-3">
                      <button
                        onClick={() => handleVerifyAlumni(selectedAlumni._id, 'approve', 'manual')}
                      ...
                      <button
                        onClick={() => handleVerifyAlumni(selectedAlumni._id, 'reject', 'manual')}
                        className={`flex-1 px-4 py-2 rounded-lg font-medium text-sm ${
                      ...
                      {selectedAlumni.aiScore && (
                        <button
                          onClick={() => handleVerifyAlumni(selectedAlumni._id, 'approve', 'ai')}
                          className={`flex-1 px-4 py-2 rounded-lg font-medium text-sm ${
