...
const UnifiedActivityAnalytics = () => {
  const [analyticsType, setAnalyticsType] = useState("alumni");
  ...
  const fetchActivityData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/analytics/activity?type=${analyticsType}`);
  ...
  // Fetch data when analytics type changes
  useEffect(() => {
    fetchActivityData();
  }, [analyticsType]);

  // Gradual data updates (simulated)
  useEffect(() => {
  ...
  const topPerformer = useMemo(() => {
    ...
    return activityData.reduce((max, curr) =>
  ...
  const filteredActivityData = useMemo(() => {
    return activityData.filter(activity => {
      ...
      if (batchFilter !== 'all' && activity.batch !== parseInt(batchFilter)) return false;
      if (departmentFilter !== 'all' && activity.department !== departmentFilter) return false;
      return true;
    });
  }, [activityData, batchFilter, departmentFilter]);
  ...
  const batches = [...new Set(activityData.map(a => a.batch))].sort();
  const departments = [...new Set(activityData.map(a => a.department))].sort();
  ...
  return (
    <div className="analytics-page">
      ...
      <div className="analytics-header">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Real-time insights into {analyticsType} engagement and activities</p>
          </div>
      ...
      <div className="flex gap-2 mb-6">
        <button
          onClick={() => setAnalyticsType("alumni")}
          className={`px-6 py-3 rounded-xl font-medium transition-all ${
            analyticsType === "alumni" 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="summary-card border border-gray-200">
            <div className="text-gray-500 text-sm">
              Total {analyticsType === "alumni" ? "Alumni" : "Students"}
            </div>
          ...
          <div className="summary-card border border-gray-200">
            ...
            <div className="text-3xl font-bold text-gray-900 mt-1">
              {activityData.length > 0 
                ? (activityData.reduce((sum, a) => sum + a.impactScore, 0) / activityData.length).toFixed(1) 
                : "0"}/10
            </div>
          ...
          <div className="summary-card border border-gray-200">
            <div className="text-gray-500 text-sm">
              {analyticsType === "alumni" ? "Top Mentor" : "Top Student"}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
            <div style={{
              ...
              background: hover.department === "CSE" ? "linear-gradient(135deg, #6366f1, #4f46e5)" : 
                         ...
                         hover.department === "ME" || hover.department === "MECH" ? "linear-gradient(135deg, #f97316, #ea580c)" : 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            <div style={{ background: '#f8fafc', padding: '10px', borderRadius: '8px' }}>
              <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '4px' }}>
                {analyticsType === "alumni" ? "Mentorships" : "Activities"}
              </div>
              <div style={{ fontWeight: 'bold', fontSize: '16px', color: '#1e293b' }}>
...
export default UnifiedActivityAnalytics;